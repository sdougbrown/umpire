---
title: 'Signals — Solid'
description: Connect @umpire/signals directly to SolidJS reactivity via reactiveUmp(). For the higher-level useUmpire() and fromSolidStore() API, see @umpire/solid instead.
---

If you want the Solid package with `useUmpire()` and `fromSolidStore()`, see [`@umpire/solid`](/adapters/solid/). Use this page when you want `reactiveUmp()` directly.
