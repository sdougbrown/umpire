...
`@umpire/zustand` connects an umpire to a store slice. It works with vanilla Zustand stores and in any environment — no React required. The demo below is a print-dialog simulator built with pure DOM helpers, a vanilla Zustand store, and `fromStore()`.
