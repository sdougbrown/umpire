...
- `fouls` is memoized from `ump.flag(previousSnapshot, currentSnapshot)`.
  ...
  return (
    <form>
      <label>
        ...
        <select
          value={plan}
          onChange={(event) => setPlan(event.currentTarget.value as SignupConditions['plan'])}
        >
      ...
      {fouls.length > 0 && <pre>{JSON.stringify(fouls, null, 2)}</pre>}
    </form>
  )
}
```
