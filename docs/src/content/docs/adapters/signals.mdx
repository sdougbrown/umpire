...
Each field is a self-contained component. When the plan signal changes, only the fields whose availability actually moved re-render — the rest stay frozen.
...
`@preact/signals-core` is the framework-agnostic signal primitive. `@preact/signals` adds Preact integration on top. Any library that implements the `SignalProtocol` interface works — Preact signals is just one option.
...
Use `reactive.foul(name)` to get the foul for a specific field, or `undefined` if none exists. This mirrors `reactive.field(name)` for availability:
