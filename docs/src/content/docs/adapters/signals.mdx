...
Each field reads the signals it touches directly during render. When the plan signal changes, only the fields whose availability actually moved re-render — the rest stay frozen.
...
function FieldControl({ field, reactive }) {
  ...
  return (
    // Preact auto-subscribes when signal-backed values are read in render.
  )
}
```

Each render reads `reactive.field(field)` and `reactive.values[field]`, which touches only that field's computed signals. The component owns its value, its controls, and its availability display. That's the natural unit of fine-grained reactivity.
