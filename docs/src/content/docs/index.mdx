...
Umpire answers one question: given the current field values and conditions, which fields should be *enabled*, **required**, or <u>flagged</u> for cleanup?
...
- **Cleanup** — yours, but guided. `flag()` recommends which stale values to reset. You decide when and whether to apply them.
</div>
