---
title: What is Umpire?
description: Umpire is a declarative field-availability engine with a zero-dependency core and adapters for React, Vue, Solid, Zustand, Redux, Zod, Effect, and more.
template: splash
hero:
...
<div class="c-umpire-minefield-callout">
  ...
  <div class="c-umpire-minefield-callout__copy">
    ...
    <a class="sl-link-button primary" href="/examples/minesweeper/">Read the writeup</a>
  </div>
</div>
...
- **Validation** — yours, but composable. [`check()`](/api/rules/#checkfield-validator) bridges validators (Zod, regex, functions) into availability rules. See [Composing with Validation](/concepts/validation/).
...
- Try the [Quick Start](/learn/) to learn each rule primitive with interactive demos.
...
- Use [umpire()](./api/) and [Rules](./api/rules/) as the reference for the core API.
...
- Robots: start with [`llms.txt`](/llms.txt) for the agent-oriented map of Umpire docs, API references, examples, and source context.
</div>
