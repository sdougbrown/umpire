...
This demo composes two layers: **Umpire** decides which fields are available (enabled, required, or due for cleanup), and **Zod** validates the ones that are. Neither knows about the other — they compose through [`@umpire/zod`](/umpire/api/rules/#typing-conditions), a thin adapter that builds a dynamic Zod schema from the availability map.
...
| What should reset after a transition? | Umpire `play()` | Company fields get reset recommendations when switching to personal |
