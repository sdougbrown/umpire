...
The actual coach layer is now separate from those visible hints. It composes the umpire plus named domain facts:

```ts
const pcCoach = createCoach({
  ump: pcUmp,
  facts: pcBuildFacts,
  getFactInput: (snapshot) => snapshot.values,
...
- `createCoach(...)` for the domain-aware layer that consumes umpire plus facts
- `createFactTable(...)` for named domain facts shared by rules, inspection, and UI
- `inspectUmpre(...)` inside the coach layer for live field and transition facts
- `createHintRuntime(...)` for latched facts plus prompt sequencing
...
That feels like a better extraction seam than the earlier history-flags helper. The reusable part is not rendering. It is “compose umpire state with domain facts, then let hints consume that richer inspection.” React can wrap the hint runtime with a hook, but hint selection itself stays derived instead of syncing through an effect.
