...
- `createReadTable(...)` for named domain reads shared by rules, scorecard, and UI, with helpers like `.from(...)` for rule predicates
...
That filtering logic is still not what Umpire is responsible for. The component keeps the product catalog in userland, but it now declares named build reads once and reuses them from `fairWhen()`, the scorecard, and render. The read table is starting to look less like a plain helper bag and more like a small constructed layer with its own baked-in helpers. Umpire owns the rule result and the cascade. The app still owns the catalog.
