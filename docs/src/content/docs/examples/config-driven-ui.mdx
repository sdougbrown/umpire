...
- [`@umpire/json`](/umpire/extensions/json/) &mdash; the portable schema in full, including `excluded` and `conditions`.
- [@umpire/dsl](/umpire/extensions/dsl/) &mdash; the pure `expr.*` vocabulary and compiler helpers.
- [Builders & Checks](/umpire/extensions/json/builders/) &mdash; JSON-specific `expr.check()`, portable validators, and rule builders.
