...
- 3ea36c4: Adds `.reason(field, expected)` and `.reasons(field, expectedArray)` to `checkAssert()` chains for asserting reason metadata alongside field availability flags.
